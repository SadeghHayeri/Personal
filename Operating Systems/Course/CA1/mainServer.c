...
int _add_contributer(Node** contributers_head, int file_index, char ip[IP_LEN], int port) {
...
// 1    DONE
int add_file_piece(File files[], char file_name[MAX_FILE_NAME], int file_index, char ip[IP_LEN], int port) {
...
int main() {
    ...
    File files[MAX_FILE_HANDLER];
    for (size_t i = 0; i < MAX_FILE_HANDLER; i++)
        strcpy(files[i].name, NOTSET);

    add_file_piece(files, "milad", 0, "172.0.0.1", 1000);
    add_file_piece(files, "sadegh", 2, "172.0.0.1", 1000);
    add_file_piece(files, "asghar", 1, "172.0.0.1", 1000);
    add_file_piece(files, "milad", 3, "172.0.0.1", 1000);

    // print_files(files);
    print_file_details(files, "milad");
