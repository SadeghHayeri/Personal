        ...
        for i in range(self.n):
            if self.table[node][i] == 1:
                if not (self.color[i] == 1 and maxDepth == 0):
                    newWay = self.solve(i, maxDepth if self.color[i] == 0 else maxDepth-1, visitList.copy())
...
if __name__ == '__main__':
    ...
    # print( g.solve( 0, 1, [0 for _ in range(6)] ) )
    ...
    print(sum( g.solve(0, maxD, [0 for _ in range(m)]) )-1)
